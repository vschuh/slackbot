...
class Response:
    ...
    def message(payload):
        ...
        if user_id != BOT_ID:
            ...
            elif threadresponses > 2 and text.lower().__contains__ ("synergy match"):
                client.reactions_add(channel=channel_id, timestamp = ts, name="nooo")
                client.chat_postMessage(channel=channel_id, thread_ts = ts, text=str("\"Manual production nothing we can do\""))
